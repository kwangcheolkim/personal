...
//
define(["jquery", "knockout", "jqueryui","jquerywidget", "globals", "general", "microsoftajax", "headerandfooter", "searchresults", "resultsData"], function ($, ko) {
...
microsoft.learning.mlx.CourseSearchViewModel = (function () {
        var cs = function (searchModel, childViewModel, overlay) {
            ...
            self.headerAndFooter = new microsoft.learning.mlx.HeaderAndFooter();
            window.headerAndFooter = window.headerAndFooter || self.headerAndFooter;

            $(document).ready(self.headerAndFooter.init(self.headerAndFooter));
            ...
            self.coursesRenderedHandler = function (elements, data) {
                if ($('#ul-courses').children().length === self.resultsSet().length) {
                    self.headerAndFooter.popUpMenusToSection('#ul-courses');
                }
            }
        };
        ...
        cs.prototype = {
            ...
            */
            updateURL: function (searchEventType) {
                ...
                //var userDataCookie = microsoft.learning.mlx.General.getUserDataCookie();
                ...
                //        userDataCookie = microsoft.learning.mlx.General.getUserDataCookie();
            ...
            */
            searchResult: function (item, parentModel, searchText, userLearningPlans, isUserMigrated, isMigrationStarted, isMigrationCompleted, searchRank) {
                ...
                self.courseDescription = ko.observable(microsoft.learning.mlx.General.removeTagsCourseDescription(item.courseShortDescription));
                self.courseLevel = ko.observable(item.courseLevel ? item.courseLevel : "");
                self.LanguageCode = ko.observable(item.languageCode);
                self.courseFriendlyName = microsoft.learning.mlx.General.getCoursePlayerFriendlyName(item.id, item.courseName);
                self.authorInfo = item.authorInfo;
                self.publishedTime = microsoft.learning.mlx.General.getDateDiffString(item.publishedTime);
                ...
                self.showInLearningPlanIcon = ko.computed(function () {
                    return (Configurations.UserId > 0 );
                });
                ...
                this.displayCourseDescription = ko.computed(function () {
                    var _description = microsoft.learning.mlx.General.removeTagsCourseDescription(self.courseDescription());

                    return microsoft.learning.mlx.General.boldSearchTerms(_description, decodeURIComponent(self.searchText()));
                });

                this.displayCourseName = ko.computed(function () {
                    var _name = microsoft.learning.mlx.General.removeTagsCourseDescription(self.courseName());

                    return microsoft.learning.mlx.General.boldSearchTerms(_name, decodeURIComponent(self.searchText()));
                });

                this.displayCourseLevel = ko.computed(function () {

                    return microsoft.learning.mlx.General.getLevelName(self.courseLevel());
                });
            },
            ...
            narrowByInfo: function (item, parentHeader) {
                ...
                this.displayName = ko.computed(function () {
                    ...
                    switch (self.parentHeader()) {
                        case SourceHeaders.LCID:
                            return microsoft.learning.mlx.General.getLevelName(self.name());
                        case SourceHeaders.CourseLevel:
                            return microsoft.learning.mlx.General.getLevelName(self.name());
                        default:
            ...
            */
            search: function (searchEventType, sourceHeader) {
                ...
                promise.done(function (results) {                    
                    ...
                    self.displayError(ResourceStrings.MVA_Search_ServerError)
                });
            ...
            */
            bindResults: function (searchEventType, sourceHeader) {
                ...
                if (!self.reSearchRequired()) {
                    ...
                    self.headerAndFooter.closePopups(true, false);

                    microsoft.learning.mlx.General.showOrHideButtonDescriptionProductTopic();
            ...
            */
            resetAllFilters: function () {
                ...
                self.headerAndFooter.closePopmenus();
            },
                        ...
                        if (element.prop("checked") === true) {
                            ...
                            for (var i = 0; i < _selectCriteriaItems.length; i++) {
                                switch (_selectCriteriaItems[i].SelectOnField) {
                                    ...
                                    case SourceHeaders.LCID:
                                        filterLanguage = self.processFilterForTracking(filterLanguage, microsoft.learning.mlx.General.getLevelName(_selectCriteriaItems[i].SelectTerm)); break;
                                }
                            }
            ...
            */
            setLanguagePreference: function (lcid, updatedLanguagePreferences) {
                ...
                if (typeof (userDataCookie) !== "undefined") {
                    ...
                    microsoft.learning.mlx.General.saveUserDataCookie(userDataCookie);
                }
            },
